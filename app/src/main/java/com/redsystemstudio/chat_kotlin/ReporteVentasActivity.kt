...
import com.redsystemstudio.chat_kotlin.Adaptadores.AdaptadorVentas
...
class ReporteVentasActivity : AppCompatActivity() {
    ...
    private lateinit var adaptador: AdaptadorVentas
    ...
    private fun setupRecyclerView() {
        adaptador = AdaptadorVentas(emptyList()) { venta ->
    ...
    private fun cargarVentas() {
        ventasRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val ventas = mutableListOf<VentaNegocio2>()
                for (ventaSnapshot in snapshot.children) {
                    val venta = ventaSnapshot.getValue(VentaNegocio2::class.java)
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@ReporteVentasActivity,
    ...
    private fun mostrarDetalleVenta(venta: VentaNegocio2) {
        ...
        val detallesVenta = mutableListOf<DetalleVentaNegocio2>()
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (detalleSnapshot in snapshot.children) {
                        val detalle = detalleSnapshot.getValue(DetalleVentaNegocio2::class.java)
                        detalle?.let {
    ...
    private fun deshacerVenta(venta: VentaNegocio2, detalles: List<DetalleVentaNegocio2>) {
        ...
        detalles.forEach { detalle ->
            productosRef.child(detalle.idProducto).get().addOnSuccessListener { snapshot ->
                ...
                producto?.let {
                    ...
                    actualizaciones["/Productos_Negocio2/${detalle.idProducto}/cantidad"] = nuevoStock
                }
    ...
    private fun eliminarVenta(
        venta: VentaNegocio2, 
        detalles: List<DetalleVentaNegocio2>,
        ...
        detalles.forEach { detalle ->
            actualizaciones["/DetallesVenta_Negocio2/${detalle.idDetalle}"] = HashMap<String, Any>()
        }

        // Eliminar la venta
        actualizaciones["/Ventas_Negocio2/${venta.idVenta}"] = HashMap<String, Any>()
    ...
    private fun agregarFilaDetalle(tableDetalles: TableLayout, detalle: DetalleVentaNegocio2) {
        productosRef.child(detalle.idProducto)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val producto = snapshot.getValue(ProductoNegocio2::class.java)
                    producto?.let {
                        val row = TableRow(this@ReporteVentasActivity).apply {
                        ...
                        row.addView(TextView(this@ReporteVentasActivity).apply {
                            text = it.nombre
